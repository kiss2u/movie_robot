...
class SiteHelper(BaseSiteHelper):
    ...
    def __init__(self, site_config, cookie_str=None, request_timeout=10.0, download_timeout=180.0, proxies=None,
        ...
        self.parser = SiteParser(site_config)
    ...
    async def search(self, keyword=None, imdb_id=None, cate_level1_list: list = None, free: bool = False,
        ...
        if self.site_config.get('search').get('pre_handler'):
            handler_name = self.site_config.get('search').get('pre_handler')
            if handler_name in search_pre_handlers:
                search_pre_handlers[handler_name].handle(search_pre_handlers[handler_name], domain=self.get_domain(),
                                                         paths=paths, query=query,
                                                         cookies=self.cookies)
        for i, p in enumerate(paths):
            if p.get('query_cates'):
                query['cates'] = self.__trans_search_cate_id__(p.get('query_cates'))
            uri = p.get('path')
